...
# Derived from M2M100 implementation posted on HuggingFace; license below:
#
# coding=utf-8
...
class M2M100SinusoidalPositionalEmbedding(nn.Module):
    ...
    Unlike BART's learned table, the weights are precomputed sinusoids and are
    NOT part of the checkpoint (registered as a non-persistent buffer), so the
    weight loader never sees them. Positions are offset by ``padding_idx + 1``
    to match HF's ``create_position_ids_from_input_ids`` convention.
    """
    ...
    def __init__(self, num_positions: int, embedding_dim: int, padding_idx: int):
        ...
        # +offset so that position p maps to row (p + offset); a small margin
        # is added for generation positions beyond max_position_embeddings.
        weights = self._build(num_positions + self.offset + 2, embedding_dim, padding_idx)
        self.register_buffer("weights", weights, persistent=False)

    @staticmethod
    def _build(num_embeddings: int, embedding_dim: int, padding_idx: int) -> torch.Tensor:
        half_dim = embedding_dim // 2
        emb = math.log(10000.0) / (half_dim - 1)
        emb = torch.exp(torch.arange(half_dim, dtype=torch.float32) * -emb)
        emb = torch.arange(num_embeddings, dtype=torch.float32).unsqueeze(1) * emb.unsqueeze(0)
        emb = torch.cat([torch.sin(emb), torch.cos(emb)], dim=1).view(num_embeddings, -1)
        if embedding_dim % 2 == 1:  # zero-pad the last dim if odd
    ...
    def forward(self, positions: torch.Tensor) -> torch.Tensor:
        """``positions`` are 0-based token positions (vLLM flattened batch)."""
        idx = positions + self.offset
        return self.weights.index_select(0, idx.view(-1).long()).to(self.weights.dtype)
...
class M2M100ParallelLMHead(ParallelLMHead):
    """
...
class M2M100Encoder(nn.Module):
    ...
    self attention layers. Each layer is a [`M2M100EncoderLayer`].
    Args:
        config: M2M100Config
    ...
    def __init__(
        ...
        # M2M-100/NLLB: a final layer norm after the stack (no layernorm_embedding)
    ...
    def forward(
        ...
        # M2M-100/NLLB applies the layer norm at the END of the encoder stack
...
class M2M100Decoder(nn.Module):
    ...
    Each layer is a [`M2M100DecoderLayer`]
    Args:
        config: M2M100Config
        embed_tokens (nn.Embedding): output embedding
    """
    ...
    def forward(
        ...
        # M2M-100/NLLB applies the layer norm at the END of the decoder stack
        hidden_states = self.layer_norm(hidden_states)
        return hidden_states
...
class M2M100ProcessingInfo(BaseProcessingInfo):
    """Processing information for M2M100 encoder-decoder models."""

    def get_hf_config(self) -> M2M100Config:
        return self.ctx.get_hf_config(M2M100Config)

    def get_supported_mm_limits(self) -> Mapping[str, int | None]:
        # M2M100's encoder input is treated as a "text" modality
        # Like M2M100, mM2M100 just has text for both encoder and decoder
        return {"text": 1}
    ...
    def get_mm_max_tokens_per_item(
        ...
        # For M2M100, the encoder can handle up to max_position_embeddings tokens
...
if hasattr(BaseProcessingInfo, "get_default_tok_params"):
    ...
    def _m2m100_get_default_tok_params(self):
        return super(M2M100ProcessingInfo, self).get_default_tok_params() \
            .with_kwargs(add_special_tokens=False)

    M2M100ProcessingInfo.get_default_tok_params = _m2m100_get_default_tok_params  # type: ignore[attr-defined]


class M2M100DummyInputsBuilder(BaseDummyInputsBuilder[M2M100ProcessingInfo]):
    """Builds dummy inputs for profiling M2M100 models."""

    def get_dummy_text(self, mm_counts: Mapping[str, int]) -> str:
        # For M2M100, the decoder prompt is separate from encoder
...
# Allow "Text" as a Multimodal Modality for M2M100.
class TextProcessorItems(ProcessorBatchItems[str]):
    """
    Data items for text modality (M2M100 encoder input is text).
    """
...
class M2M100MultiModalProcessor(EncDecMultiModalProcessor[M2M100ProcessingInfo]):
    """Multimodal processor for M2M100 encoder-decoder models."""
    ...
    def _call_hf_processor(
        ...
        """
        M2M100 doesn't have a HuggingFace Processor — it only has a tokenizer.
        ...
        # For M2M100 encoder-decoder: check if we have encoder text data
    ...
    def _get_mm_fields_config(
        ...
        # For M2M100, encoder_input_ids from tokenization are the encoder input
    ...
    def _get_prompt_updates(
        ...
        # For M2M100, we need to replace the placeholder [0] with the actual
...
class M2M100ForConditionalGeneration(nn.Module, SupportsQuant, SupportsMultiModal):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        config = vllm_config.model_config.hf_config
        lora_config = vllm_config.lora_config
        # currently all existing M2M100 models have `tie_word_embeddings` enabled
        ...
        self.lm_head = M2M100ParallelLMHead(
            config.vocab_size, config.d_model, embed_scale=embed_scale
        )
        ...
        self._encoder_max_seq_padding = _env_flag(
            "VLLM_M2M100_ENCODER_MAX_SEQ_PADDING", default=False
        )
        self._pad_id = getattr(self.config, "pad_token_id", None)
        if self._encoder_max_seq_padding and self._pad_id is None:
            logger.warning(
                "Pad token id is not set; disabling VLLM_M2M100_ENCODER_MAX_SEQ_PADDING."
            )
    ...
    def embed_multimodal(self, **kwargs) -> MultiModalEmbeddings:
        ...
        if not self._encoder_max_seq_padding:
            ...
            # NOTE (NickLucche): Basic encoder batching optimization: M2M100 input sequences
            ...
            if "final_logits_bias" in name:
                ...
                if shared_embedding_weight is not None:
                    logger.warning(
                        ...
                        "%s, skipping. This is expected on facebook/m2m100-large"
